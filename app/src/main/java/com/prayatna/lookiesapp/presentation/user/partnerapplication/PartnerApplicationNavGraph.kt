...
import com.prayatna.lookiesapp.utils.NavGraphRoutes
...
fun NavGraphBuilder.partnerApplicationNavGraph(navController: NavController) {
    navigation(
        ...
        route = NavGraphRoutes.PARTNER_APPLICATION_FLOW
    ) {
        composable(NavigationRoutes.ADD_LOCATION) { backStackEntry ->
            val mainEntry = remember(backStackEntry) {
                navController.getBackStackEntry(NavGraphRoutes.PARTNER_APPLICATION_FLOW)
            }
